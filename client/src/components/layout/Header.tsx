...
export default function Header({ headerData }: H) {
    ...
    return (
        <section className="relative w-screen h-[80vh] flex  items-center z-0 ">
            <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-t from-black from-5% to-transparent to-80% z-1"></div>
            <video className="h-full  w-screen object-cover absolute top-0 left-0" autoPlay loop muted>
                <source src={`http://localhost:1337${headerData.background?.url}`} type="video/mp4" />
                Your browser does not support the video tag.
            </video>
            <div className="z-10 wrapper w-full h-full flex flex-col justify-center items-start">
