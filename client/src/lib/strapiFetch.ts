...
export async function fetchStrapiProducts(endpoint: string) {
  try {
    const res = await fetch(
      `http://localhost:1337/api/products?populate[image][fields][0]=url&populate[cta][fields][0]=text&populate[cta][fields][1]=href`
    );
    if (!res.ok) {
      throw new Error(`Fail to fetch ${endpoint} `);
    }
